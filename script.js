...
(function() {
    ...
    const observer = new IntersectionObserver((entries) => {
      for (const entry of entries) {
        if (entry.isIntersecting) {
          ...
          document.querySelectorAll('.ca-toc-item').forEach(a => a.classList.remove('ca-toc-active'));
  ...
  // ========== MAIN ==========
  function main() {
    const text = prompt('Paste your Workday Student Academic History transcript below:');
    if (!text || text.trim().length === 0) { alert('No transcript provided. Cancelled.'); return; }
    const transcript = parseTranscript(text);
    if (transcript.length === 0) { alert('Could not parse any courses from the transcript.\nMake sure you copy from Workday Academic History.'); return; }
    ...
    alert('Core Annotator loaded!\n\nCourses found: ' + transcript.length +
      '\nK&U Hours: ' + analysis.kuSummary.totalHours + '/' + analysis.kuSummary.requiredHours +
      '\nK&U Categories: ' + analysis.kuSummary.categoriesUsed + '/' + analysis.kuSummary.requiredCategories);
  }
