(()=>{function I(o){let c=o.replace(/\u00a0/g," ").replace(/\r\n?/g,`
`).replace(/\s+/g," ").trim(),i=/\b([A-Z]{2,5})\s+(\d{3}[A-Z]?)\s*-\s*(.+?)\s+([A-Z]{1,2}[+-]?)\s+\d+(?:\.\d+)?\s+(\d+(?:\.\d+)?)\s+\d+(?:\.\d+)?\b/g,p=[],l=i.exec(c);for(;l!==null;){let[,u,b,a,x,m]=l;p.push({code:u+" "+b,title:a.replace(/\s+/g," ").trim(),grade:x,hours:parseFloat(m)}),l=i.exec(c)}let n=new Map,k=[];for(let u of p){let b=u.code.match(/^([A-Z]{2,5}\s+\d{3})L$/);b?k.push({parentCode:b[1],...u}):n.set(u.code,{...u})}for(let u of k)n.has(u.parentCode)?n.get(u.parentCode).hours+=u.hours:n.set(u.code,u);return Array.from(n.values())}function L(o){return"ca-"+o.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/-+/g,"-").replace(/^-|-$/g,"")}function N(o){o||(o=document);let c=o.querySelectorAll("div.acalog-core"),i=[],p="";for(let l=0;l<c.length;l++){let n=c[l],k=n.querySelector("h2"),u=n.querySelector("h3"),b=n.querySelector("h4"),a=n.querySelector("p"),x=a?a.textContent.replace(/\u00a0/g," ").trim():"",m=k||u||b;if(k&&!u&&!b){p=k.textContent.trim(),m&&(m.id=L(p)),i.push({type:"section-header",section:p,instructions:x,element:n,coreIndex:l,slug:L(p)});continue}let C="";m&&(C=Array.from(m.childNodes).filter(v=>v.nodeType===3||v.nodeType===1&&v.tagName==="A").map(v=>v.textContent.trim()).join("").trim(),m.id=L(C));let w=b!==null,y=x.match(/(\d+)[–-](\d+)\s+semester hours/),E=x.match(/up to (\d+)\s+semester hours in any one discipline/),$=y?parseInt(y[1]):null,H=y?parseInt(y[2]):null,f=E?parseInt(E[1]):null,e=$!==null?"hour-range":"pick-one",s=n.querySelector("ul"),g=[],t=[],r=null;if(s){for(let v of s.children)if(v.classList.contains("acalog-adhoc")){let A=v.textContent.trim();["Or","One from","And"].includes(A)||(r=A,t.push({name:A,element:v}))}else if(v.classList.contains("acalog-course")){let A=v.querySelector("a"),S=(A?A.textContent.trim():"").match(/^([A-Z]{2,5})\s+(\d{3}[A-Z]?)\s*-\s*(.+)$/);S&&g.push({code:S[1]+" "+S[2],title:S[3].trim(),subDiscipline:r,element:v})}}i.push({type:w?"tag":"category",section:p,name:C,instructions:x,reqType:e,minHours:$,maxHours:H,maxPerDiscipline:f,courses:g,subDisciplineElements:t,element:n,coreIndex:l,slug:L(C)})}return i}var B=["World Languages II","Arts, Oral Rhetoric, Visual Rhetoric","Humanities","Mathematical Sciences","Natural Sciences","Social and Behavioral Sciences"];function O(o,c){let i=new Set(c.map(a=>a.code)),p={};c.forEach(a=>{p[a.code]=a});let l={};for(let a of o)if(!(!a.courses||!a.name))for(let x of a.courses)l[x.code]||(l[x.code]=[]),l[x.code].push(a.name);let n={},k=0,u=0,b=[];for(let a of o){if(a.type==="section-header"){b.push({...a,status:"header"});continue}if(!a.courses||a.courses.length===0){b.push({...a,status:"info-only"});continue}let x=a.courses.filter(f=>i.has(f.code)),m=0,C={};for(let f of x){let e=p[f.code],s=e?e.hours:0;m+=s;let g=f.subDiscipline||"General";C[g]=(C[g]||0)+s}let w=a.maxHours!==null?Math.min(m,a.maxHours):m,y=new Set;if(a.maxPerDiscipline)for(let[f,e]of Object.entries(C))e>=a.maxPerDiscipline&&y.add(f);let E=a.courses.some(f=>!i.has(f.code)&&!y.has(f.subDiscipline||"General")),$;if(a.reqType==="pick-one")$=x.length>0?"complete":"incomplete";else{let f=a.minHours||0;f>0?m<f?$="incomplete":a.maxPerDiscipline&&a.maxHours!==null&&w<a.maxHours&&E?$="satisfied":$="complete":$=m>0?"complete":"optional-available"}B.includes(a.name)&&(n[a.name]={raw:m,effective:w,max:a.maxHours},k+=w,m>0&&u++);let H=a.courses.map(f=>{let e=i.has(f.code),s=l[f.code]||[],g=s.filter(t=>t!==a.name);return{...f,isTaken:e,isMultiSection:s.length>1,otherCategories:g,isMaxedDiscipline:y.has(f.subDiscipline||"General")&&!e,grade:e&&p[f.code]?.grade||"",hours:e&&p[f.code]?.hours||0}});b.push({...a,status:$,hoursEarned:m,hoursEffective:w,hoursByDiscipline:C,maxedDisciplines:y,takenCount:x.length,courseAnnotations:H})}return{results:b,transcript:c,kuSummary:{totalHours:k,requiredHours:26,categoriesUsed:u,requiredCategories:5,hoursByCategory:n}}}var d={taken:"#2d8a4e",takenBg:"#e6f7ed",opportunity:"#b8860b",opportunityBg:"#fff8dc",doubleCount:"#1a73e8",doubleCountBg:"#e8f0fe"};function T(){["ca-styles","ca-toc-styles"].forEach(o=>{document.getElementById(o)?.remove()}),document.querySelectorAll(".ca-legend,.ca-summary-panel,.ca-badge,.ca-hours-note,#ca-toc,.ca-disclaimer,.ca-transcript-section").forEach(o=>{o.remove()}),document.querySelectorAll('[class*="ca-course-"],[class*="ca-category-"]').forEach(o=>{o.className=o.className.replace(/ca-[\w-]+/g,"").trim(),o.removeAttribute("data-ca-note")})}function R({results:o,kuSummary:c,transcript:i}){T();let p=document.createElement("style");p.id="ca-styles",p.textContent=`
  ...
  `,document.head.appendChild(l);let n=document.querySelector("div.acalog-core");if(!n)return;let k=document.createElement("div");k.className="ca-legend",k.innerHTML=`
    ...
    <div class="ca-legend-item"><div class="ca-legend-swatch" style="border-left:3px solid ${d.opportunity}"></div>Open opportunities</div>
    <div class="ca-legend-item"><div class="ca-legend-swatch" style="background:#eee;opacity:0.35"></div>Maxed / satisfied</div>
  `;let u=document.createElement("div");u.className="ca-summary-panel";let b=Math.min(100,Math.round(c.totalHours/c.requiredHours*100)),a=c.totalHours>=c.requiredHours?"#4ade80":"#fbbf24",x=c.categoriesUsed>=c.requiredCategories?"#4ade80":"#fbbf24",m="";for(let e of B){let s=c.hoursByCategory[e]||{raw:0,effective:0,max:null},g=s.raw>0?"complete":"incomplete",t=s.max!==null&&s.raw>s.max?`<div class="cap-note">${s.raw}h earned, capped at ${s.max}h</div>`:"",r=L(e);m+=`<a href="#${r}" class="ca-summary-item ${g}" data-slug="${r}">
  ...
  `,document.head.appendChild(o)}function z(o,c){let i=document.getElementById(h.status);i&&(i.textContent=o,i.className="",c==="success"&&i.classList.add("ca-status-success"),c==="error"&&i.classList.add("ca-status-error"))}function F(){let o=new URL(window.location.href);return o.pathname===D.pathname&&o.searchParams.get("catoid")===D.catoid&&o.searchParams.get("navoid")===D.navoid}function q(o){let c=(o||"").trim();if(c.length===0){z("Paste your Workday Academic History transcript to see your core opportunities.",null);return}let i=I(c);if(i.length===0){T(),z("Could not parse any courses. Copy from Workday Academic Record > Academic History and paste the full content here.","error");return}let p=N(),l=O(p,i);R(l),z("Progress updated: "+i.length+" courses, "+l.kuSummary.totalHours+"/"+l.kuSummary.requiredHours+" K&U hours, "+l.kuSummary.categoriesUsed+"/"+l.kuSummary.requiredCategories+" K&U categories.","success"),console.log("[Core Annotator]",i.length,"courses:",i.map(n=>n.code+"("+n.hours+"h)")),console.log("[Core Annotator] K&U:",l.kuSummary)}function U(){if(document.getElementById(h.panel))return;let o=document.querySelector("div.acalog-core");if(!o||!o.parentElement)return;j();let c=document.createElement("section");c.id=h.panel,c.innerHTML=`
    <details id="${h.details}">
      ...
      <div class="ca-launcher-actions">
        ...
        <span id="${h.status}">Paste your Workday Academic History transcript to see your core opportunities.</span>
      </div>
    </details>`,o.parentElement.insertBefore(c,o);let i=c.querySelector("details"),p=c.querySelector("textarea");c.addEventListener("click",l=>{let n=l.target.closest("button[data-action]");n&&(n.dataset.action==="analyze"&&q(p.value),n.dataset.action==="reset"&&(p.value="",T(),z("Annotations cleared.",null),i.open=!1))}),p.addEventListener("paste",l=>{let n=l.clipboardData?.getData("text/plain");if(typeof n=="string"&&n.length>0){l.preventDefault(),p.value=n,q(n),i.open=!1;return}window.setTimeout(()=>{q(p.value),i.open=!1},0)})}function M(){if(console.log("v0.0.3"),!!F()){if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",U,{once:!0});return}U()}}window.CalvinCoreAnnotator={main:M};M();})();
